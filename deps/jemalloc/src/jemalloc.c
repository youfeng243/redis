...
JEMALLOC_EXPORT JEMALLOC_ALLOCATOR
...
== NULL)) {
if (
config_xmalloc &&unlikely(opt_xmalloc)
) {
...
if (
config_stats &&likely(ret != NULL)
) {
assert(usize
==
isalloc(ret, config_prof
));
*
tsd_thread_allocatedp_get(tsd)
+=
usize;
}
UTRACE(0, size, ret);
JEMALLOC_VALGRIND_MALLOC(ret
!= NULL, ret, usize, false);
return (ret);
}
...
if (
unlikely((err = imemalign(&ret, alignment, size, 1)
) != 0)) {
ret = NULL;
set_errno(err);
}
JEMALLOC_VALGRIND_MALLOC(err
== 0, ret,
isalloc(ret, config_prof
),
false);
...
ifree(tsd_t *tsd, void *ptr, tcache_t *tcache) {
    ...
    UNUSED
    size_t
    rzsize JEMALLOC_CC_SILENCE_INIT(0);
...
je_realloc(void *ptr, size_t size) {
    ...
    UNUSED
    size_t
    old_rzsize JEMALLOC_CC_SILENCE_INIT(0);
...
UTRACE(0, size, p);
JEMALLOC_VALGRIND_MALLOC(true, p, usize,
    ...
    if (likely(chunk != ptr)) { /* indication that this is not a HUGE alloc */
        ...
        if (likely((mapbits & CHUNK_MAP_LARGE) == 0)) { /* indication that this is not a LARGE alloc */
            ...
            if (chunk != (arena_chunk_t *) CHUNK_ADDR2BASE(bin->runcur)) {
                ...
                size_t
                availregs = bin_info->nregs * bin->stats.curruns;
