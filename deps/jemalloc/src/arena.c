...
*
arena_miscelm_key_create(size_t
size)
{

return ((arena_chunk_map_misc_t *)(
arena_mapbits_size_encode(size)
|
CHUNK_MAP_KEY));
}
...
JEMALLOC_INLINE_C size_t
arena_miscelm_size_get(arena_chunk_map_misc_t
*miscelm)
{
arena_chunk_t *chunk;
...
return (
arena_mapbits_size_decode(mapbits)
);
}
...
if (size <=
small_maxrun &&small_run_tab[size >> LG_PAGE]
)
return (size);
...
arena_avail_comp(arena_chunk_map_misc_t * a, arena_chunk_map_misc_t * b) {
    ...
    uintptr_t a_miscelm = (uintptr_t) a;
    size_t
    a_qsize = run_quantize(arena_miscelm_is_key(a) ?
                           arena_miscelm_key_size_get(a) : arena_miscelm_size_get(a));
    size_t
    b_qsize = run_quantize(arena_miscelm_size_get(b));
...
arena_run_split_remove(arena_t *arena, arena_chunk_t *chunk, size_t run_ind,
    ...
    if (rem_pages > 0) {
        ...
        size_t
        flag_unzeroed_mask = (flags == 0) ? CHUNK_MAP_UNZEROED :
                             0;
...
arena_run_split_large_helper(arena_t *arena, arena_run_t *run, size_t size,
    ...
    arena_chunk_map_misc_t * miscelm;
    size_t
    flag_dirty, flag_decommitted, run_ind, need_pages;
    size_t
    flag_unzeroed_mask;
...
arena_run_split_small(arena_t *arena, arena_run_t *run, size_t size,
    ...
    arena_chunk_map_misc_t * miscelm;
    size_t
    flag_dirty, flag_decommitted, run_ind, need_pages, i;
    ...
    for (i = 0; i < need_pages; i++) {
        size_t
        flag_unzeroed = arena_mapbits_unzeroed_get(chunk,
                                                   run_ind + i);
...
arena_chunk_init_hard(arena_t *arena) {
    ...
    bool zero, commit;
    size_t
    flag_unzeroed, flag_decommitted, i;
...
arena_chunk_ralloc_huge_expand(arena_t *arena, void *chunk, size_t oldsize,
    ...
    udiff = usize - oldsize;
    size_t
    cdiff = CHUNK_CEILING(usize) - CHUNK_CEILING(oldsize);
...
arena_run_first_best_fit(arena_t *arena, size_t size) {
    size_t
    search_size = run_quantize_first(size);
...
arena_purge_stashed(arena_t *arena, chunk_hooks_t *chunk_hooks,
    ...
    for (rdelm = qr_next(purge_runs_sentinel, rd_link),
        ...
        if (rdelm == &chunkselm->rd) {
            ...
            size_t
            pageind, run_size, flag_unzeroed, flags, i;
            bool decommitted;
            arena_chunk_t *chunk =
                    (arena_chunk_t *) CHUNK_ADDR2BASE(rdelm);
            arena_chunk_map_misc_t * miscelm =
...
arena_run_coalesce(arena_t *arena, arena_chunk_t *chunk, size_t *p_size,
    ...
    if (run_ind + run_pages < chunk_npages &&
        ...
        flag_decommitted) {
        size_t
        nrun_size = arena_mapbits_unallocated_size_get(chunk,
                                                       run_ind + run_pages);
        size_t
        nrun_pages = nrun_size >> LG_PAGE;
    ...
    if (run_ind > map_bias && arena_mapbits_allocated_get(chunk,
                                                          run_ind - 1) == 0 &&
        arena_mapbits_dirty_get(chunk, run_ind - 1) ==
        flag_dirty && arena_mapbits_decommitted_get(chunk, run_ind - 1) ==
                      flag_decommitted) {
        size_t
        prun_size = arena_mapbits_unallocated_size_get(chunk,
                                                       run_ind - 1);
        size_t
        prun_pages = prun_size >> LG_PAGE;
...
arena_run_decommit(arena_t *arena, arena_chunk_t *chunk, arena_run_t *run) {
    ...
    run_ind = arena_miscelm_to_pageind(miscelm);
    size_t
    offset = run_ind << LG_PAGE;
    size_t
    length = arena_run_size_get(arena, chunk, run, run_ind);
...
arena_run_dalloc(arena_t *arena, arena_run_t *run, bool dirty, bool cleaned,
    ...
    arena_chunk_map_misc_t * miscelm;
    size_t
    size, run_ind, run_pages, flag_dirty, flag_decommitted;
...
arena_run_trim_head(arena_t *arena, arena_chunk_t *chunk, arena_run_t *run,
    ...
    pageind = arena_miscelm_to_pageind(miscelm);
    size_t
    head_npages = (oldsize - newsize) >> LG_PAGE;
    size_t
    flag_dirty = arena_mapbits_dirty_get(chunk, pageind);
    size_t
    flag_decommitted = arena_mapbits_decommitted_get(chunk, pageind);
    size_t
    flag_unzeroed_mask = (flag_dirty | flag_decommitted) == 0 ?
                         CHUNK_MAP_UNZEROED : 0;
...
arena_run_trim_tail(arena_t *arena, arena_chunk_t *chunk, arena_run_t *run,
    ...
    if (config_debug) {
        UNUSED
        size_t
        tail_npages = (oldsize - newsize) >> LG_PAGE;
        assert(arena_mapbits_large_size_get(chunk,
                                            pageind + head_npages + tail_npages - 1) == 0);
        assert(arena_mapbits_dirty_get(chunk,
                                       pageind + head_npages + tail_npages - 1) == flag_dirty);
...
arena_alloc_junk_small(void *ptr, arena_bin_info_t *bin_info, bool zero) {
    ...
    if (zero) {
        size_t
        redzone_size = bin_info->redzone_size;
...
arena_dalloc_large_locked_impl(arena_t *arena, arena_chunk_t *chunk,
    ...
    if (config_fill || config_stats) {
        size_t
        usize = arena_mapbits_large_size_get(chunk, pageind) -
                large_pad;
...
arena_ralloc_large_grow(arena_t *arena, arena_chunk_t *chunk, void *ptr,
    ...
    pageind = ((uintptr_t) ptr - (uintptr_t) chunk) >> LG_PAGE;
    size_t
    npages = (oldsize + large_pad) >> LG_PAGE;
    size_t
    followsize;
    ...
    if (oldsize + followsize >= usize_min) {
        ...
        arena_run_t *run;
        size_t
        usize, splitsize, size, flag_dirty, flag_unzeroed_mask;
...
arena_lg_dirty_mult_default_set(ssize_t lg_dirty_mult) {
    ...
    if (!arena_lg_dirty_mult_valid(lg_dirty_mult))
        return (true);
    atomic_write_z((size_t * ) & lg_dirty_mult_default, (size_t)
    lg_dirty_mult);
    return (false);
}
...
bin_info_run_size_calc(arena_bin_info_t *bin_info) {
    ...
    if (config_fill && unlikely(opt_redzone)) {
        size_t
        align_min = ZU(1) << (jemalloc_ffs(bin_info->reg_size) -
                              1);
