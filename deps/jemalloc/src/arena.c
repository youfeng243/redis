...
void
arena_basic_stats_merge(UNUSED tsdn_t
...
*nmuzzy) {
*nthreads +=
arena_nthreads_get(arena, false
);
*
...
arena_decay_reinit(arena_decay_t *decay, ssize_t decay_ms) {
    ...
    if (decay_ms > 0) {
        nstime_init(&decay->interval, (uint64_t)
        decay_ms *
        KQU(1000000));
    ...
    decay->jitter_state = (uint64_t)(uintptr_t)
    decay;
    arena_decay_deadline_init(decay);
    decay->nunpurged = 0;
...
arena_decay_ms_valid(ssize_t decay_ms) {
    ...
    if (decay_ms == -1 || (uint64_t)
        decay_ms <= NSTIME_SEC_MAX *
                    KQU(1000)) {
        return true;
    }
    return false;
...
arena_dalloc_junk_small_t *JET_MUTABLE
...
static void
arena_bin_lower_slab(UNUSED tsdn_t

*tsdn,
arena_t *arena, extent_t
*slab,
bin_t *bin
) {
assert(extent_nfree_get(slab) > 0);
 ...
 */
if (bin->slabcur != NULL &&
extent_snad_comp(bin
->slabcur, slab) > 0) {
/* Switch slabcur. */
if (
extent_nfree_get(bin
->slabcur) > 0) {
arena_bin_slabs_nonfull_insert(bin, bin
->slabcur);
} else {
arena_bin_slabs_full_insert(arena, bin, bin
->slabcur);
}
bin->
slabcur = slab;
