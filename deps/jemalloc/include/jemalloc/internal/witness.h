...
*
witness_tsd_tsdn(witness_tsd_t
*witness_tsd) {
return (witness_tsdn_t *)
witness_tsd;
}
...
*
witness_tsdn_tsd(witness_tsdn_t
*witness_tsdn) {
assert(!
witness_tsdn_null(witness_tsdn)
);
return &witness_tsdn->
witness_tsd;
}
...
extern witness_lock_error_t *JET_MUTABLE
witness_lock_error;

typedef void (witness_owner_error_t)(const witness_t *);

extern witness_owner_error_t *JET_MUTABLE
witness_owner_error;

typedef void (witness_not_owner_error_t)(const witness_t *);

extern witness_not_owner_error_t *JET_MUTABLE
witness_not_owner_error;

typedef void (witness_depth_error_t)(const witness_list_t *,
                                     witness_rank_t rank_inclusive, unsigned depth);

extern witness_depth_error_t *JET_MUTABLE
witness_depth_error;
...
static inline bool
        witness_owner(witness_tsd_t * witness_tsd,
...
static inline void
        witness_assert_owner(witness_tsdn_t * witness_tsdn,
const witness_t *witness
) {
witness_tsd_t *witness_tsd;
...
if (
witness_owner(witness_tsd, witness
)) {
return;
}
...
static inline void
        witness_assert_depth(witness_tsdn_t * witness_tsdn, unsigned
depth) {
witness_assert_depth_to_rank(witness_tsdn,
WITNESS_RANK_MIN, depth);
}
...
witness_lock(witness_tsdn_t * witness_tsdn, witness_t * witness) {
    ...
    if (w == NULL) {
        ...
        witness_lock_error(witnesses, witness);
                                                               ...
                                                               witness->comp ||
                                            w->comp(w, w->opaque, witness, witness->opaque) >
                                            0)) {
        /*
