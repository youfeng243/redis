...
rtree_cache_direct_map(uintptr_t
key) {
unsigned ptrbits = ZU(1) << (LG_SIZEOF_PTR + 3);
unsigned cumbits = (rtree_levels[RTREE_HEIGHT - 1].cumbits -
                    rtree_levels[RTREE_HEIGHT - 1].bits);
...
#  ifdef RTREE_LEAF_COMPACT
JEMALLOC_ALWAYS_INLINE uintptr_t
rtree_leaf_elm_bits_read(tsdn_t
*tsdn,
rtree_t *rtree, rtree_leaf_elm_t
*elm,
bool dependent
) {
...
*
rtree_leaf_elm_bits_extent_get(uintptr_t
bits) {
#    ifdef __aarch64__
/*
...
rtree_leaf_elm_bits_szind_get(uintptr_t
bits) {
return (szind_t)(bits >> LG_VADDR);
}

JEMALLOC_ALWAYS_INLINE bool
rtree_leaf_elm_bits_slab_get(uintptr_t
bits) {
...
JEMALLOC_ALWAYS_INLINE extent_t
*
rtree_leaf_elm_extent_read(UNUSED
tsdn_t *tsdn, UNUSED
rtree_t *rtree,
        rtree_leaf_elm_t
*elm,
...
return
rtree_leaf_elm_bits_extent_get(bits);
#else
...
JEMALLOC_ALWAYS_INLINE szind_t
rtree_leaf_elm_szind_read(UNUSED
tsdn_t *tsdn, UNUSED
rtree_t *rtree,
        rtree_leaf_elm_t
*elm,
...
uintptr_t bits = rtree_leaf_elm_bits_read(tsdn, rtree, elm, dependent);
return
rtree_leaf_elm_bits_szind_get(bits);
#else
...
JEMALLOC_ALWAYS_INLINE bool
rtree_leaf_elm_slab_read(UNUSED
tsdn_t *tsdn, UNUSED
rtree_t *rtree,
        rtree_leaf_elm_t
*elm,
...
uintptr_t bits = rtree_leaf_elm_bits_read(tsdn, rtree, elm, dependent);
return
rtree_leaf_elm_bits_slab_get(bits);
#else
...
static inline void
rtree_leaf_elm_extent_write(UNUSED tsdn_t

*tsdn,
UNUSED rtree_t
*rtree,
rtree_leaf_elm_t *elm, extent_t
*extent) {
#ifdef RTREE_LEAF_COMPACT
...
static inline void
rtree_leaf_elm_szind_write(UNUSED tsdn_t

*tsdn,
UNUSED rtree_t
*rtree,
rtree_leaf_elm_t *elm, szind_t
szind) {
assert(szind <= NSIZES);
...
static inline void
rtree_leaf_elm_slab_write(UNUSED tsdn_t

*tsdn,
UNUSED rtree_t
*rtree,
rtree_leaf_elm_t *elm, bool
slab) {
#ifdef RTREE_LEAF_COMPACT
uintptr_t old_bits = rtree_leaf_elm_bits_read(tsdn, rtree, elm,
                                              true);
...
*
rtree_leaf_elm_lookup(tsdn_t
*tsdn,
rtree_t *rtree, rtree_ctx_t
*rtree_ctx,
uintptr_t key, bool
dependent,
bool init_missing
...
for (
unsigned i = 1;
i < RTREE_CTX_NCACHE_L2; i++) {
RTREE_CACHE_CHECK_L2(i);
}
#undef RTREE_CACHE_CHECK_L2

return
rtree_leaf_elm_lookup_hard(tsdn, rtree, rtree_ctx, key,
...
return
rtree_leaf_elm_extent_read(tsdn, rtree, elm, dependent
);
}

JEMALLOC_ALWAYS_INLINE szind_t
rtree_szind_read(tsdn_t
*tsdn,
rtree_t *rtree, rtree_ctx_t
*rtree_ctx,
uintptr_t key, bool
...
return
rtree_leaf_elm_szind_read(tsdn, rtree, elm, dependent
);
}
...
JEMALLOC_ALWAYS_INLINE bool
rtree_extent_szind_read(tsdn_t
*tsdn,
rtree_t *rtree, rtree_ctx_t
*rtree_ctx,
uintptr_t key, bool
dependent,
extent_t **r_extent, szind_t
*r_szind) {
rtree_leaf_elm_t *elm = rtree_read(tsdn, rtree, rtree_ctx, key,
                                   dependent);
if (!dependent && elm == NULL) {
...
JEMALLOC_ALWAYS_INLINE bool
rtree_szind_slab_read(tsdn_t
*tsdn,
rtree_t *rtree, rtree_ctx_t
*rtree_ctx,
uintptr_t key, bool
dependent,
szind_t *r_szind, bool
*r_slab) {
rtree_leaf_elm_t *elm = rtree_read(tsdn, rtree, rtree_ctx, key,
                                   dependent);
if (!dependent && elm == NULL) {
