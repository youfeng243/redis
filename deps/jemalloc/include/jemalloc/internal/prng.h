...
JEMALLOC_ALWAYS_INLINE uint32_t
prng_lg_range_u32(atomic_u32_t
*state,
unsigned lg_range,
bool atomic
) {
...
atomic_compare_exchange_weak_u32(state, &state0,
...
atomic_compare_exchange_weak_zu(state, &state0,
        state1, ATOMIC_RELAXED, ATOMIC_RELAXED
));
} else {
state1 = prng_state_next_zu(state0);
