...
return
arena_prof_tctx_get(tsdn, ptr, alloc_ctx
);
}

JEMALLOC_ALWAYS_INLINE void
        prof_tctx_set(tsdn_t * tsdn,
const void *ptr, size_t
usize,
alloc_ctx_t *alloc_ctx, prof_tctx_t
*tctx) {
cassert(config_prof);
assert(ptr != NULL);
...
JEMALLOC_ALWAYS_INLINE bool
        prof_sample_accum_update(tsd_t * tsd, size_t
usize,
bool update,
        prof_tdata_t
**tdata_out) {
...
if (update) {
tdata->bytes_until_sample -=
usize;
}
return true;
} else {
if (
tsd_reentrancy_level_get(tsd)
> 0) {
return true;
}
...
if (
prof_sample_accum_update(tsd, usize,
true, NULL)) {
/*
