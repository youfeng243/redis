...
static unsigned
        tree_recurse(node_t * node, unsigned
black_height,
...
/* Self. */
assert_u32_eq(node
->magic, NODE_MAGIC, "Bad magic");

/* Left subtree. */
if (left_node != nil)
ret +=
tree_recurse(left_node, black_height, black_depth, nil
);
else
...
if (right_node != nil)
ret +=
tree_recurse(right_node, black_height, black_depth, nil
);
else
ret += (black_depth != black_height);
...
static void
        node_remove(tree_t * tree, node_t * node, unsigned
nnodes)
{
node_t *search_node;
...
imbalances = tree_recurse(tree->rbt_root, black_height, 0,
                          &(tree->rbt_nil));
assert_u_eq(imbalances,
0, "Tree is unbalanced");
assert_u_eq(tree_iterate(tree), nnodes
-1,
"Unexpected node iteration count");
assert_u_eq(tree_iterate_reverse(tree), nnodes
-1,
"Unexpected node iteration count");
}
